...
if [[ `uname -s` == *CYGWIN* ]] ; then
  ...
  if [ `uname -s` = Linux ] ; then
    ...
    if [ -e "$HIGHEST_PRIORITY_JAVA_9" ] ; then
      export JAVA_HOME="${HIGHEST_PRIORITY_JAVA_9%/bin/javac}"
    elif ! $JAVA_HOME/bin/java -version 2>&1 | head -n 1 | grep "9" >> /dev/null ; then
      echo "Please set JAVA_HOME to version 9"
      exit
    fi
  else
    if [ `uname -s` = Darwin ] ; then
      export JAVA_HOME=`/usr/libexec/java_home -F -v9*`
    else
...
SBT_LAUNCH=$HOME/.sbt/sbt-launch-1.0.4.jar
URL='https://repo.scala-sbt.org/scalasbt/maven-releases/org/scala-sbt/sbt-launch/1.0.4/sbt-launch-1.0.4.jar'
